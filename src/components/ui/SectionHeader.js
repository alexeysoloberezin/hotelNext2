...
function SectionHeader({title, text, btnHref,wrapperClassName,isCenter, bigMob, color = 'black', mobImage, classText}) {
  const colors = {
    'black': {
      'text': 'text-black',
      'btn': 'white'
  ...
  return (
    <div className={clsx(' md:mb-[50px] mb-[30px]', colors[color].text)}>
      ...
      <div className={clsx("flex md:items-center md:flex-row flex-col  gap-[20px]", wrapperClassName)}>
        ...
        {mobImage && <Zoom><Image src={mobImage} noWebp={true} alt="" className={"max-h-[600px] min-h-[340px] object-cover md:hidden"}/></Zoom>}
        {btnHref
